<%@ Page Language="C#" AutoEventWireup="true" CodeFile="addadvisor.aspx.cs" Inherits="KBC.add_advisor" MasterPageFile="~/Site.Master" %>
...
<asp:Content ID="MainContent" ContentPlaceHolderID="MainContent" runat="server">
  <div class="crud-section">
    <h2>Add Advisor</h2>
    <asp:Label ID="lblAdvisorMsg" runat="server" ForeColor="Red" />
    <div class="crud-form" style="flex-direction: column; align-items: stretch;">
      <asp:TextBox ID="txtAdvisorName" runat="server" placeholder="Name" />
      <asp:TextBox ID="txtAdvisorRole" runat="server" placeholder="Role (e.g., Faculty Advisor, Industry Mentor)" />
      <asp:TextBox ID="txtAdvisorExpertise" runat="server" placeholder="Expertise (e.g., Entrepreneurship, Finance)" />
      <asp:TextBox ID="txtAdvisorDept" runat="server" placeholder="Department" />
      <asp:TextBox ID="txtAdvisorEmail" runat="server" placeholder="Email" />
      <asp:TextBox ID="txtAdvisorBio" runat="server" placeholder="Short bio (optional)" TextMode="MultiLine" Rows="3" />
      <div style="display: flex; gap: 12px; align-items: center;">
        <label style="color: #cbd5e1; font-size: 14px;">Photo:</label>
        <asp:FileUpload ID="fuAdvisorPhoto" runat="server" accept="image/*" />
      </div>
      <asp:HiddenField ID="hfEditingAdvisorId" runat="server" Value="0" />
      <div style="display: flex; gap: 12px;">
        <asp:Button ID="btnAddAdvisor" runat="server" Text="Add Advisor" OnClick="btnAddAdvisor_Click" />
        <asp:Button ID="btnCancelEdit" runat="server" Text="Cancel" OnClick="btnCancelEdit_Click" style="background: #64748b; display: none;" />
      </div>
    </div>

    <h3>Advisors</h3>
    <asp:GridView ID="gvAdvisors" runat="server" AutoGenerateColumns="False" DataKeyNames="Id" OnRowDeleting="gvAdvisors_RowDeleting" OnRowEditing="gvAdvisors_RowEditing" CssClass="crud-table" GridLines="None" Width="100%">
      <Columns>
